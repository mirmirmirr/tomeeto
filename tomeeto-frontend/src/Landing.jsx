    ...
    <div className={`relative flex flex-col items-center justify-center min-h-screen p-4 ${isDarkMode ? 'bg-[#3E505B]' : 'bg-[#F5F5F5]'}`}>
      ...
      <div className="flex flex-col items-center space-y-6 p-6 w-full max-w-lg">
        ...
        <div className="flex space-x-4 w-full">
          <button className={`flex-1 px-6 py-3 bg-transparent border-2 rounded-md shadow-md transition duration-300 ${isDarkMode ? 'text-white border-white' : 'text-black border-black'}`}>LOGIN</button>
          <button className="flex-1 px-6 py-3 bg-[#FF5C5C] text-white rounded-md shadow-md transition duration-300">SIGNUP</button>
        </div>
      </div>
    </div>
  );
